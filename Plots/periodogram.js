...
function periodogram(chartID) {
    ...
    var margin = { top: 5, right: 30, bottom: 50, left: 60, between: 5 },
        ...
        startTime = new Date(),
        putcontrols = "",
        dataSource = {name: "main data", table: "", dates: "", values: "", colour:""},
...
//TODO - date formatting, and make this starttime
    ...
    function calcQp(values, varPer, recordingLengthHrs) {
        ...
        const avgP = foldedValues.map(col => col.reduce((a, b) => a + b) / col.length);
        const numerator = avgP.map(p => (p - avgAll) ** 2).reduce((a, b) => a + b);
        const denom = values.map(v => (v - avgAll) ** 2).reduce((a, b) => a + b) / (rowNum * colNum * rowNum);
