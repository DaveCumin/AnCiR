import {
  ...
  query,
  w2grid,
} from "../dependencies/w2ui-2.0.es6.min.js";
...
let tabs = new w2tabs({
  ...
  onClick(event) {
    ...
    query("#plotContent").html(
      `<div id="${charts[index].selection.split("#")[1]}"></div>`
    );

    d3.select(charts[index].selection).call(charts[index].chart);
  ...
  onClose(event) {
    if (tabs.tabs.length === 1) {
      document.getElementById("plotContent").innerHTML =
        "Create a plot to show it here.";
      document.getElementById("controls").innerHTML = "";
      document.getElementById("exportplotbutton").innerHTML = "";
    }

    //FIXME: need to remove the chart and adjust accordingly
    charts.splice(
      this.tabs.findIndex((t) => t.id == event.target),
      1
    );
  },
});
...
window.addTab = function () {
  ...
  if (tabs.tabs.length === 0) {
    document.getElementById(
      "exportplotbutton"
    ).innerHTML = `   <button onclick="exportSVG()">EXPORT PLOT</button>
                                                                    <hr>`;
  }
  ...
  onClose(event) {
    if (datatabs.tabs.length === 1) {
      document.getElementById("dataContent").innerHTML =
        "Click on a data link to show it here.";
...
window.adddataTab = function (from, index, chartDataIndex, chartDataName) {
  ...
  if (datatabs.tabs.length === 0) {
    document.getElementById(
      "exportdatabutton"
    ).innerHTML = `   <button onclick="exportDataCSV()">EXPORT as csv</button>
                                                                    <hr>`;
  }

  //now deal with the new tab
  if (
    datatabs.tabs.filter(
      (d) =>
        d.id ===
        "datatab_" +
          from +
    ...
    //if it already exists in the tabs do noting
    ...
    if (from === "chartList") {
      ...
      datatabs.add({
        id:
          ...
          "_" +
          chartDataName,
        text: "Plot " + chartDataIndex + ": " + chartDataName,
        closable: true,
      });
    }
  ...
  datatabs.click(
    "datatab_" + from + "_" + index + "_" + chartDataIndex + "_" + chartDataName
  );
};
